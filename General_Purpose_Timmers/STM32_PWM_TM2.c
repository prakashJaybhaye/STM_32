...
#define TIM2_CCR1 (*(volatile uint32_t *)(TIM2_BASE + 0x38))
...
{
    RCC_AHB1ENR |= (1 << 0);
    GPIOA_MODER &= ~(3 << (2 * LED_PA0_AF));
    GPIOA_MODER |= (2 << (2 * LED_PA0_AF));
    GPIOA_AFRL &= ~(0xF);
    GPIOA_AFRL |= (1 << 0);

}

void Init_TM_2_PWM(void)
{
    RCC_APB1ENR |= (1 << 0);
    TIM2_PSC = 15;
    TIM2_ARR = 1000;
...
int main(void)
{
   volatile uint8_t duty_cycle = 0;
    ...
    while (1)
    {
        TIM2_CCR1 = (TIM2_ARR*duty_cycle) / 100;
        delay_ms(500);
        duty_cycle += 20;
